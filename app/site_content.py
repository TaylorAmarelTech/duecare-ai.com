...
COMPONENTS: tuple[Component, ...] = (
    Component(
        "Runtime",
        "Live",
        "Runs Gemma 4 through Kaggle, local, llama.cpp, HF Spaces, edge boxes, or mobile LiteRT.",
        "Render does not host GPU inference; it links the places where the model actually runs.",
    ),
    Component(
        "Harness",
        "Live",
        "Deterministic safety layer: GREP rules, RAG context, contacts, tools, personas, and graders.",
        "This is the visible Kaggle chat-package demo and the core proof that the model is grounded.",
    ),
    Component(
        "Eval",
        "Live",
        "Rubrics, adversarial probes, regression checks, and reproducible notebook outputs.",
        "Judges can inspect the notebooks and compare stock Gemma 4 against the enhanced harness.",
    ),
    Component(
        "Exchange",
        "Prototype",
        "Privacy-preserving hub for anonymized signals, pack metadata, prompts, and evaluation manifests.",
        "This website is the first public slice: aggregate-only intake and no raw case management.",
    ),
    Component(
        "Sentinel",
        "Prototype",
        "OpenClaw/OpenCrawl-style public-source update proposals for curator review.",
        "Crawlers can propose changes; humans approve signed knowledge-pack releases.",
    ),
    Component(
        "Trainer",
        "Prototype",
        "Gemma 4 adaptation path: approved examples, LoRA, eval gates, GGUF, and HF Hub releases.",
        "Training remains behind strict PII gates; the public hub only shows metadata and status.",
    ),
    Component(
        "Channels",
        "Roadmap",
        "Messenger, WhatsApp, SMS, web, and embassy-portal adapters for NGOs and regulators.",
        "Draft-only handoff is the boundary: Duecare drafts; users or trusted caseworkers decide.",
    ),
    Component(
        "Mobile",
        "Sibling project",
        "Worker-owned private assistant for on-device checks and localized rights information.",
        "The mobile story is privacy-first: sensitive content stays on the worker's device.",
    ),
)
...
USE_CASES: tuple[CatalogItem, ...] = (
    CatalogItem("Worker-side private checker", "on device", "A worker pastes a suspicious message and receives rights-aware, localized guidance without sending raw content to the hub."),
    CatalogItem("NGO caseworker copilot", "trusted review", "A caseworker uses the harness to draft explanations, identify indicators, and route a complaint to the right public channel."),
    CatalogItem("Government regulator dashboard", "aggregate trends", "A regulator monitors anonymized patterns by corridor and sees which knowledge packs need updating."),
    CatalogItem("Platform safety escalation", "public ads", "A job platform screens suspicious posts, warns users, and sends aggregate pattern signals instead of raw messages."),
    CatalogItem("Research reproducibility", "Kaggle", "Judges and researchers rerun notebooks to verify prompts, evaluations, and model behavior from source artifacts."),
)
...
def home_html() -> str:
    ...
    <div class=\"flow\">
      <div class=\"node\"><span class=\"status\">Kaggle</span><h3>Gemma 4 + Harness</h3><p>Runs the visible chat demo, GREP rules, RAG context, contacts, tools, and evaluation rubrics.</p></div>
      <div class=\"node\"><span class=\"status\">Website</span><h3>Duecare Exchange</h3><p>Accepts anonymized patterns and public-source update proposals for curator review.</p></div>
      <div class=\"node\"><span class=\"status\">Partners</span><h3>NGOs + Regulators</h3><p>Use signed knowledge packs, complaint-channel context, and draft-only handoff flows.</p></div>
      <div class=\"node\"><span class=\"status\">Private edge</span><h3>Mobile + Local</h3><p>Keeps sensitive worker messages on-device or inside trusted NGO systems.</p></div>
    </div>
    ...
    <div class=\"grid\">
      ...
      <article class=\"card\"><h3>Use cases</h3><p>Worker-side, NGO, regulator, platform, and research deployment stories.</p><a href=\"/use-cases\">Open use cases →</a></article>
    </div>
    """
...
def components_html() -> str:
    body = f"""
    <h1>Eight components, one privacy boundary.</h1>
    <p class=\"lead\">The public website should make the platform legible in seconds: live core, prototype hub, and honest roadmap.</p>
...
def use_cases_html() -> str:
    body = f"""
    <h1>Use cases that fit the privacy rule.</h1>
    <p class=\"lead\">Duecare is not a public raw-case intake portal. It is infrastructure for safer advice, evaluation, and coordination around Gemma 4.</p>
