...
Auto-deploy:    Yes, on push to production
Persistent disk:
...
1. Create or connect the GitHub repo `TaylorAmarelTech/duecare-ai.com`.
2. Create a Render Web Service from the repo.
3. Use Docker runtime.
4. Set branch to `production`.
5. Add a persistent disk mounted at `/app/.duecare`.
6. Set health check path to `/api/health`.
7. Add custom domains:
   - `duecare-ai.com`
   - `www.duecare-ai.com`
8. Configure DNS using Render's records.
9. Confirm TLS certificate issuance.
